...
- **numpy split RESOLVED on numpy 1.26.4** (Card 5): coherent set = scispaCy 0.6.2 + spaCy 3.7.5 + thinc 8.2.5 + scipy 1.17.1 + transformers 5.12.1 + torch cu128 + chromadb + hi-ml-multimodal, ALL import together. The Card-3 numpy-2 was a detour; the real issue was a scipy/numpy mismatch (fixed by a numpy-1 scipy). [Source: empirical — full-stack import verified, Card 5]
- **scispaCy install pins spaCy 3.7.5 + numpy<2** via the `en_core_sci_*` model (no spaCy-3.8 sci model exists). Don't force spaCy 3.8 — it breaks the sci model. graphrag's `spacy~=3.8`/`numpy~=2.1` pins stay unsatisfiable; address only when Card 10 (GraphRAG) lands. [Source: empirical, Card 5]
